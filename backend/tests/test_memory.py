...
@pytest.mark.parametrize("entrypoint", ["backend.fastapi.main", "llm.chat"])
...
def test_llm_layer_does_not_import_the_web_app():
    ...
    llm/ used to import backend.constants, which imported the settings module --
    so touching the LLM layer configured the whole application as a side effect.
    """
    resident = _modules_after_importing("llm.chat")
    assert "fastapi" not in resident
    assert "backend.fastapi.core.init_settings" not in resident
...
def test_connection_pool_is_sized_for_an_idle_service():
    ...
    from backend.fastapi.dependencies import database
