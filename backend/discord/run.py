...
def start_or_resume_session(
        ...
        user_input: str,
        is_group: bool,
        is_new_session: bool) -> dict:
    try:
        with rollback_manager() as resources_to_rollback:
            user_id = get_or_create_user(user_discord_id, user_name, resources_to_rollback)
            current_session = manage_session(channel_id, user_id, is_group, is_new_session, resources_to_rollback)

            # Create a new conversation
            new_conversation = create_conversation(current_session["id"], resources_to_rollback)

            # Create a new message
            new_message = create_message(channel_id, current_session["id"], new_conversation["id"], user_id, "user", user_input, resources_to_rollback)

            # Add the document in the melisearch index
            document_data = [{
                "conversation_id": new_conversation["id"],
                "user_input": user_input,
                "session_id": current_session["id"]
            }]
            _ = insert_documents(current_session["id"], document_data) if is_group else insert_documents(user_id, document_data)
...
def complete_session_chat(
    ...
    try:
        with rollback_manager() as resources_to_rollback:
            # Start the session flow and get the session and conversation
            session_result = start_or_resume_session(channel_id, user_discord_id, user_name, user_input, is_group, is_new_session)
    ...
    if resume_session_id:
        try:
            with rollback_manager() as resources_to_rollback:
                ...
                current_sessions = find_active_sessions(channel_id, user_id, is_group)
...
def search_messages_and_list_sessions(
    channel_id: str,
    user_discord_id: str,
    user_name: str,
    user_input: str,
    ...
    try:
        with rollback_manager() as resources_to_rollback:
            ...
            # Step 2: Seacrh for Documents
            documents = hybrid_search(channel_id, user_input) if is_group else hybrid_search(user_id, user_input)
